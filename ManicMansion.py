...
class GameBoard(GameObject):
    ...
    def draw_objects(self, surface, x, y, width, height, color):
            pg.draw.rect(surface, color, pg.Rect(x, y, width, height))
...
while run:
    ...
    for ghost in ghosts:
        ...
        game_board.draw_objects(surface, ghost.x, ghost.y, width, height, BLUE)
    
    #Tegner alle blokkene
    for block in blocks:
        game_board.draw_objects(surface, block.x, block.y, width, height, GRAY)
    ...
    for sheep in sheep_list:
        game_board.draw_objects(surface, sheep.x, sheep.y, width, height, WHITE)
    ...
    if carrying_sheep is False:
        player.reduce_speed()
        game_board.draw_objects(surface, player.x, player.y, player.width, player.height, RED)
    # Hvis spilleren bærer sauen, går spilleren fra å være rød til å være hvit
    else:
        game_board.draw_objects(surface, player.x, player.y, player.width, player.height, WHITE)
